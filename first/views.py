...
class Index(APIView):
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid(): 
            ...
            command = f'ffmpeg -i {video} -i {img_logo} -i {usr_img} -filter_complex \"[0:v][1:v]overlay=10:10[bg];[bg][2:v]overlay={width-150}:8\ , drawtext=text=\'{context}\':fontsize=100:x=w-mod(t*100\,w+tw):y=h/1.2-th/2:fontcolor=RED:box=1:boxcolor=black@0.5" -codec:a copy {output_file}'
            os.system(command)
